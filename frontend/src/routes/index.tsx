...
import { allTeamsQueryOptions } from "../queries/teams"; // <--- Import Query Options

export const Route = createFileRoute("/")({
  component: Home,
  // 1. Pre-fetch data so it's ready when component mounts
  loader: ({ context }) =>
...
function Home() {
  ...
  // 2. Read from the cache (Suspense will handle loading states)
  const { data: teams } = useSuspenseQuery(allTeamsQueryOptions);

  const showOnboarding = teams.length === 0;
        ...
        {teams.length === 0 ? (
          ...
          <div className="relative bg-linear-to-br from-gray-900/90 via-gray-900/60 to-gray-950 border border-gray-800 rounded-xl p-12 text-center">
            <div className="max-w-md mx-auto">
              ...
              <p className="text-gray-500 mb-6">
                You haven't joined or created any teams yet. Choose an option
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {teams.map((team, index) => (
              ...
              >
                {/* Card Container */}
                <div className="group relative bg-linear-to-br from-gray-900/90 via-gray-900/60 to-gray-950 border border-gray-800 rounded-xl p-6 shadow-2xl hover:border-gray-700 hover:scale-[1.02] transition-all duration-300 h-full flex flex-col">
                  {/* Team Name - Truncated with Fade/Ellipsis */}
                  <h3
                    className="text-2xl font-bold text-white mb-4 truncate"
                  ...
                  <div className="flex items-center gap-3 pt-4 border-t border-gray-800/50">
                    ...
                    <Link
                      ...
                      className="p-2.5 text-gray-400 hover:text-white bg-gray-800/50 hover:bg-gray-700 border border-gray-700 hover:border-gray-600 rounded-lg transition-colors"
                      title="Team Settings"
                    >

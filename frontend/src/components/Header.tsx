...
export const Header: React.FC = () => {
  ...
  // We only need account and connectWallet now
  ...
  // Check if we're on the settings or docs page
  const isOnSettingsPage = routerState.location.pathname.includes("/settings");
  const isOnDocsPage = routerState.location.pathname.includes("/docs");

  // Get team name from router state if available
  const teamName = (
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between p-4 lg:px-6 lg:py-4 border-b border-gray-800 bg-gray-950/95 backdrop-blur-sm">
      ...
      <div className="flex items-center gap-3">
        ...
        {isInTeamContext && (
          <>
            {isOnSettingsPage || isOnDocsPage ? (
              // Show Dashboard button when on settings or docs page
              <button
                onClick={() =>
              ...
              <>
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-green-300 bg-green-600/20 hover:bg-green-600/30 border border-green-600/50 hover:border-green-500 rounded-lg transition-colors"
                ...
                <button
                  onClick={() =>
                    navigate({ to: `/$teamId/settings`, params: { teamId } })
                  }
                  className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-300 bg-gray-800/50 hover:bg-gray-700/50 border border-gray-700/50 hover:border-gray-600 rounded-lg transition-colors group"
                  title="Team Settings"
                >
